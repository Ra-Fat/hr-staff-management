...
class StaffService(BaseService[StaffRepository]):
    ...
    def __init__(self, repository: StaffRepository, department_repository: DepartmentRepository) -> None:
    ...
    async def create(self, data: StaffCreate):
        payload = data.model_dump(exclude={"department_uuid", "email"}, exclude_none=True)
    ...
    async def update(self, uuid: Any, data: StaffUpdate, not_found_msg: Optional[str] = None):
        ...
        updates = data.model_dump(exclude={"department_uuid"}, exclude_none=True)
    ...
    async def get_my_profile(self, user_id: int):
        instance = await self.repository.get_by_user_id(user_id)
        if not instance:
